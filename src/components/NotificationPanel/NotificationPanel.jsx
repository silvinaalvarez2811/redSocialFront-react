...
const NotificationPanel = ({ notifs, onClose }) => {
    ...
    const height= notifs?.length > 0 ? "h-96" : "h-fit"
    ...
    return (
        <div ref={panelRef} className={`flex w-96 absolute top-16 bg-zinc-900 text-slate-300 flex-col shadow-xl p-3 rounded-lg ${height}`}>
            <h2 className="pt-2 text-xl pl-2">Notificaciones</h2>
            {notifs?.length > 0 ? (
                <div className="mt-2 overflow-auto">{
                    notifs.map((notf, index) => (
                        <Link className="group" key={index} to={`/request/${notf.postId._id}/${notf.from._id}`}>
                            <div className="flex rounded-lg items-center p-2 group-hover:bg-slate-800">
            ...
            ) : <p className="text-slate-500 text-md">No tienes notificaciones</p>}
        </div>
    )
}
